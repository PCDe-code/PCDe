...
#    random.shuffle(the_test_list)
   ...
   ############### chen #####################
    #将item分为流行POI和unpop POI
    valid_list_item = []
    f = open("valid_list_item.txt", "r")
    for x in f:
        valid_list_item.append(x)
    f.close()
    print(len(valid_list_item))
    all_valid_item=[]
    for i in valid_list_item:
        j=i.strip().split(' ')
        # print(j)
        for x in j: 
            # print(x)      
            all_valid_item.append(x)
    all_valid_item[0]

    from collections import Counter

    pop_result = Counter(all_valid_item)
    pop_result
    pop_result=sorted(pop_result.items(),key=lambda x:x[1],reverse=True)
    pop_result[0]
    pop_POI=[]
    for i in range(int(len(valid_list_item)*0.2)+1):
        pop_POI.append(pop_result[i][0])
    valid_pop_POI=pop_POI
    ##################################################
    #chen
    ...
    if A.eval == 1:
        ...
        if A.mod == 'ours':
            #chen
            fp = './data/PN-model-ours/v4-code-stable-s-0-e-171-lr-0.001-gamma-0-playby-policy-stra-maxsim-topK-3-trick-0-eval-0-init-0-mini-1-always-1-upcount-4-upreg-0.001-m-0-epi-60-1127jianReward.txt'
            #fp = './data/PN-model-ours/PN-model-ours.txt'#ori
        if A.initeval == 1:
...
#                print (k)
        ...
        if cfg.item_dict[str(item_id)]['POI_Type'] is not None:
            choose_pool = ['clusters', 'POI_Type'] + big_feature_list #ori
            #choose_pool = ['clusters'] + big_feature_list # not ask type
        ...
        for c in choose_pool:
            ...
            if A.purpose != 'pretrain':
                log_prob_list, rewards, success, turn_count, known_feature_category = run_one_episode(current_transE_model, user_id, item_id, A.mt, False, write_fp,
                                                         ...
                                                         optimizer1_transE, optimizer2_transE, A.alwaysupdate, start_facet, A.mask, sample_dict, choose_pool_original,features, items,valid_pop_POI)
            else:
...
#Can test random selection.
                        ...
                        print('prob_star_list',prob_star_list)
...
#        
        ...
        if epi_count % check_span == 0 and epi_count >= 3 * check_span and cfg.eval != 1 and A.purpose != 'pretrain':
            PATH = './data/PN-model-{}/ypev4-code-{}-s-{}-e-{}-lr-{}-gamma-{}-playby-{}-stra-{}-topK-{}-trick-{}-eval-{}-init-{}-mini-{}-always-{}-upcount-{}-upreg-{}-m-{}-epi-{}-1127jianReward.txt'.format(
                A.mod.lower(), A.code, A.startFrom, A.endAt, A.lr, A.gamma, A.playby, A.strategy, A.TopKTaxo, A.trick,
                A.eval, A.initeval,
